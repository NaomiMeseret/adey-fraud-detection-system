...
class DataLoader:
    ...
    def load_ecommerce_data(self, filename: str = "Fraud_Data.csv") -> pd.DataFrame:
        ...
        if missing_columns:
            raise ValueError(
                f"Missing required columns in e-commerce data: {missing_columns}"
            )
    ...
    def load_creditcard_data(self, filename: str = "creditcard.csv") -> pd.DataFrame:
        ...
        if missing_columns:
            raise ValueError(
                f"Missing required columns in credit card data: {missing_columns}"
            )
    ...
    def load_ip_country_mapping(
        ...
        if missing_columns:
            raise ValueError(
                f"Missing required columns in IP mapping data: {missing_columns}"
            )
    ...
    def validate_data_quality(self, df: pd.DataFrame, dataset_type: str) -> dict:
        ...
        if dataset_type == "ecommerce":
            ...
            if "class" in df.columns:
                ...
                results["imbalance_ratio"] = class_dist.get(0, 0) / max(
                    class_dist.get(1, 1), 1
                )
