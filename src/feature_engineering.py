...
class FeatureEngineer:
    ...
    def convert_ip_to_int(self, ip_str: str) -> Optional[int]:
        """
    ...
    def add_time_since_signup(self, df: pd.DataFrame,
        ...
        # Add binary feature for very recent signups (< 1 hour)
        df['is_very_recent_signup'] = (df['time_since_signup_hours'] < 1).astype(int)
        
        logger.info(f"Added time_since_signup features")
        return df
    
    def add_transaction_frequency(self, df: pd.DataFrame,
                                 user_col: str = 'user_id',
                                 time_col: str = 'purchase_time',
                                 windows_hours: List[int] = [1, 24, 168]) -> pd.DataFrame:
        """
        ...
        # Sort by user and time for efficient window calculations
        ...
        for window in windows_hours:
            ...
            for user_id in df[user_col].unique():
                ...
                for idx in df[user_mask].index:
                    ...
                    count = ((user_times >= window_start) & 
                            (user_times < current_time)).sum()
                    df.loc[idx, col_name] = count
        
        logger.info(f"Added transaction frequency features for windows: {windows_hours}")
        return df
    ...
    def map_ip_to_country(self, df: pd.DataFrame,
        ...
        # Convert IP ranges to integers in mapping
        ip_mapping_df = ip_mapping_df.copy()
        ip_mapping_df['lower_bound_int'] = ip_mapping_df['lower_bound_ip_address'].apply(self.convert_ip_to_int)
        ip_mapping_df['upper_bound_int'] = ip_mapping_df['upper_bound_ip_address'].apply(self.convert_ip_to_int)
        
        # Remove rows with invalid IP mappings
        ip_mapping_df = ip_mapping_df.dropna(subset=['lower_bound_int', 'upper_bound_int'])
    ...
    def add_country_risk_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate country-level fraud statistics
        country_stats = df.groupby('country').agg({
            'class': ['count', 'sum', 'mean']
        }).reset_index()
        
        country_stats.columns = ['country', 'total_transactions', 'fraud_count', 'fraud_rate']
        
        # Merge back to main DataFrame
        df = df.merge(country_stats[['country', 'fraud_rate']], on='country', how='left')
        
        # Fill missing fraud rates with overall rate
        overall_fraud_rate = df['class'].mean()
        df['fraud_rate'] = df['fraud_rate'].fillna(overall_fraud_rate)
        
        # Add high-risk country flag (top 10% by fraud rate)
        fraud_rate_threshold = country_stats['fraud_rate'].quantile(0.9)
        df['is_high_risk_country'] = (df['fraud_rate'] >= fraud_rate_threshold).astype(int)
    ...
    def encode_categorical_features(self, df: pd.DataFrame,
        ...
        # One-hot encode
        df_encoded = pd.get_dummies(df, columns=categorical_cols, drop_first=drop_first, prefix_sep='_')
    ...
    def scale_numerical_features(self, df: pd.DataFrame,
        ...
        if numerical_cols is None:
            # Auto-detect numerical columns (exclude target and ID columns)
            exclude_cols = ['class', 'Class', 'user_id', 'device_id', 'ip_address', 'ip_int']
